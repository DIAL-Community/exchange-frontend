...
describe('Unit test for the contact card', () => {
  test('Check contact card will display name, email, title on "list" mode.', () => {
    ...
    expect(component.getByTestId('contactName')).toHaveClass('font-semibold')
    expect(component.getByTestId('contactEmail')).toHaveClass('text-sm')
    expect(component.getByTestId('contactTitle')).toHaveClass('text-sm')
