export const paginatedDatasets = {
  'data': {
    'paginatedDatasets': [
      {
        'id': '159',
        'name': 'Advocacy Training for Community Health Workers',
        'slug': 'advocacy_training_for_community',
        'imageFile': '/assets/datasets/content_placeholder.svg',
        'datasetType': 'dataset',
        'parsedDescription': 'A free digital training that equips Community Health Workers (CHWs)',
        'datasetDescription': {
          'id': '6228',
          'description': 'A free digital training that equips Community Health Workers (CHWs)',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '23'
          },
          {
            'id': '21'
          }
        ],
        'sectors': [],
        'tags': []
      },
      {
        'id': '153',
        'name': 'African Drone and Data Academy',
        'slug': 'african_drone_and_data_academy',
        'imageFile': '/assets/datasets/content_placeholder.svg',
        'datasetType': 'dataset',
        'parsedDescription': `
          MOOCs that provide youth with 21st-century skills related to drone technology and data analysis
        `,
        'datasetDescription': {
          'id': '6219',
          'description': 'MOOCs that provide youth with 21st-century skills related to drone technology and data analysis',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '28'
          },
          {
            'id': '21'
          }
        ],
        'sectors': [],
        'tags': []
      },
      {
        'id': '38',
        'name': 'African Storybook',
        'slug': 'african_storybook',
        'imageFile': '/assets/datasets/content_placeholder.svg',
        'datasetType': 'dataset',
        'parsedDescription': 'Provides open access to picture storybooks.',
        'datasetDescription': {
          'id': '6217',
          'description': 'Provides open access to picture storybooks.',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '28'
          }
        ],
        'sectors': [
          {
            'id': '198'
          }
        ],
        'tags': []
      },
      {
        'id': '20',
        'name': 'African Terrestrial Fibre Network Mapping Project',
        'slug': 'african_terrestrial_fibre_networ',
        'imageFile': '/assets/datasets/dataset_placeholder.svg',
        'datasetType': 'dataset',
        'parsedDescription': 'Open Source map of fibre optic infrastructure projects across the African continent.',
        'datasetDescription': {
          'id': '80',
          'description': 'Open Source map of fibre optic infrastructure projects across the African continent.',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '24'
          }
        ],
        'sectors': [
          {
            'id': '246'
          }
        ],
        'tags': []
      },
      {
        'id': '1',
        'name': 'AfricaRISING Dataverse',
        'slug': 'africarising_dataverse',
        'imageFile': '/assets/products/africarising_dataverse.png',
        'parsedDescription': 'Africa Research in Sustainable Intensification for the Next Generation (Africa RISING)',
        'datasetType': 'dataset',
        'datasetDescription': {
          'id': '1655',
          'description': 'Africa Research in Sustainable Intensification for the Next Generation (Africa RISING)',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '32'
          },
          {
            'id': '32'
          }
        ],
        'sectors': [
          {
            'id': '178'
          }
        ],
        'tags': []
      },
      {
        'id': '61',
        'name': 'AI Agro',
        'slug': 'ai_agro',
        'imageFile': '/assets/products/ai_agro.png',
        'parsedDescription': 'Open Source Precision Agriculture with Drones leverages a set of machine learning algorithms',
        'datasetType': 'dataset',
        'datasetDescription': {
          'id': '6142',
          'description': 'Open Source Precision Agriculture with Drones leverages a set of machine learning algorithms',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '32'
          },
          {
            'id': '24'
          },
          {
            'id': '30'
          }
        ],
        'sectors': [],
        'tags': []
      },
      {
        'id': '78',
        'name': 'Angaza Elimu',
        'slug': 'angaza_elimu',
        'imageFile': '/assets/datasets/content_placeholder.svg',
        'parsedDescription': 'Adaptive assessments for better learning outcomes',
        'datasetType': 'dataset',
        'datasetDescription': {
          'id': '3085',
          'description': 'Adaptive assessments for better learning outcomes',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '28'
          }
        ],
        'sectors': [],
        'tags': []
      },
      {
        'id': '160',
        'name': 'Antura and the Letters',
        'slug': 'antura_and_the_letters',
        'imageFile': '/assets/products/antura_and_the_letters.png',
        'parsedDescription': ' Open source smartphone application that teaches children how to read in Arabic.',
        'datasetType': 'dataset',
        'datasetDescription': {
          'id': '6230',
          'description': ' Open source smartphone application that teaches children how to read in Arabic.',
          'locale': 'en'
        },
        'sdgs': [
          {
            'id': '28'
          }
        ],
        'sectors': [],
        'tags': []
      }
    ]
  }
}

export const datasetPaginationAttribute = {
  'data': {
    'paginationAttributeDataset': {
      'totalCount': 165
    }
  }
}
