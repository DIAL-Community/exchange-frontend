...
const ContactBio = ({ user, contact }) => {
  ...
  const generateEditPath = () => {
    if (asPath.indexOf('/hub/admin/users') >= 0) {
      return `/hub/admin/users/${user.id}/edit-contact`
    } else if (asPath.indexOf('/hub/admin') >= 0) {
      return '/hub/admin/profile/edit-contact'
    } else {
      return '/hub/dashboard/profile/edit-contact'
    }
  }
