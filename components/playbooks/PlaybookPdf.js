...
const MoveContent = ({ move, format }) => {
  return (
    <div>
      <div className='font-semibold px-4 py-4 my-auto'><Text style={styles.moveHeader}>{move.name}</Text></div>
      <div className='flex flex-col border border-dial-orange-light'>
        <div className='px-4 py-4'>
          <div className='fr-view text-dial-gray-dark'>
            <Text style={styles.moveDesc}>{move?.moveDescription && parse(move.moveDescription.description)}</Text>
          </div>
          {
            move?.resources && move?.resources.length > 0 &&
              <>
                <div className='font-semibold py-2'><Text style={styles.resourcesHeader}>{format('move.resources.header')}</Text></div>
                          ...
                          <div key={resource.i} className='group border-2 border-gray-300 hover:border-dial-yellow card-drop-shadow'>
                            <div className='flex'>
                              <div className='flex flex-col gap-2 px-3 py-4'>
                                <div className='font-semibold'><Text style={styles.link}>{resource.name}</Text></div>
                              </div>
      ...
      <Page style={styles.page} size="A4">
        <IntlProvider locale={ locale }>
          <div className='my-4 mx-2'>
            <div className='h1 mb-4 h-12'><Text style={styles.h1}>{format('playbooks.label')}: {data.playbook.name}</Text></div>
            <div className='h4'><Text style={styles.h4}>{format('playbooks.overview')}</Text></div>
            <div className='fr-view tiny-editor text-dial-gray-dark'>
              <Text style={styles.desc}>{parse(data.playbook.playbookDescription.overview)}</Text>
            </div>
            <div className='h4'><Text style={styles.h4}>{format('playbooks.audience')}</Text></div>
            <div className='fr-view tiny-editor text-dial-gray-dark'>
              <Text style={styles.desc}>{parse(data.playbook.playbookDescription.audience)}</Text>
            </div>
            <div className='h4'><Text style={styles.h4}>{format('playbooks.outcomes')}</Text></div>
            <div className='fr-view tiny-editor text-dial-gray-dark'>
              <Text style={styles.desc}>{parse(data.playbook.playbookDescription.outcomes)}</Text>
            </div>
