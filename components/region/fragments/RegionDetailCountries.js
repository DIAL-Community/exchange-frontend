...
import { UPDATE_REGION_COUNTRIES } from '../../shared/mutation/region'
import { COUNTRY_SEARCH_QUERY } from '../../shared/query/country'
import { DisplayType } from '../../utils/constants'
import { fetchSelectOptions } from '../../utils/search'

const RegionDetailCountries = ({ region, editingAllowed, headerRef }) => {
  const { formatMessage } = useIntl()
  const format = useCallback((id, values) => formatMessage({ id }, values), [formatMessage])

  const client = useApolloClient()

  const [countries, setCountries] = useState(region.countries)
  ...
  const [updateRegionCountries, { loading, reset }] = useMutation(UPDATE_REGION_COUNTRIES, {
    onError() {
      setIsDirty(false)
    ...
    onCompleted: (data) => {
      const { updateRegionCountries: response } = data
      if (response?.region && response?.errors?.length === 0) {
        setIsDirty(false)
        setCountries(response?.region?.countries)
...
export default RegionDetailCountries
