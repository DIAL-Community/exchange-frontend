...
import { UPDATE_DATASET_SECTORS } from '../../shared/mutation/dataset'
...
const DatasetDetailSectors = ({ dataset, canEdit }) => {
  ...
  const [updateDatasetSectors, { loading, reset }] = useMutation(UPDATE_DATASET_SECTORS, {
    onError() {
      setIsDirty(false)
      setSectors(dataset?.sectors)
      showFailureMessage(format('toast.submit.failure', { entity: format('ui.sector.header') }))
      reset()
    },
    onCompleted: (data) => {
      const { updateDatasetSectors: response } = data
      if (response?.dataset && response?.errors?.length === 0) {
        setIsDirty(false)
        setSectors(response?.dataset?.sectors)
