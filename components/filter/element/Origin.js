...
export const OriginAutocomplete = (props) => {
  ...
  const { origins, setOrigins, containerStyles, controlSize } = props
    ...
    <>
      {
        origins &&
          origins.map(origin => (
            <div key={`filter-${origin.label}`} className='px-2 py-1 my-auto rounded-md bg-dial-yellow text-sm text-dial-gray-dark'>
