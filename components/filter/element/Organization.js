...
export const OrganizationAutocomplete = ({
  ...
  const controlPlaceholder = placeholder ??
    format('filter.byEntity', {
      entity: format(aggregatorOnly ? 'ui.aggregator.label' : 'ui.organization.label')
    })
    ...
    <div className={classNames(containerStyles)} data-testid='organization-search'>
      <Select
        ...
        aria-label={format('filter.byEntity', {
          entity: aggregatorOnly ? format('ui.aggregator.label') : format('ui.organization.label')
        })}
        ...
        noOptionsMessage={() => {
          format('filter.searchFor', {
            entity: format(aggregatorOnly ? 'ui.aggregator.header' : 'ui.organization.header')
          })
        }}
      ...
      {organizations?.map((organization, organizationIdx) => (
        <div className='py-1' key={organizationIdx}>
          <Pill
            ...
            label={`
              ${aggregatorOnly
              ? format('ui.aggregator.label')
              : format('ui.organization.label')}: ${organization.label}
            `}
