    ...
    <Transition appear show={isOpen} as={Fragment} data-testid='dialog'>
      <DialogHeadlessui initialFocus={initialFocusRef} as='div' className='fixed z-100' onClose={onClose} >
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          ...
          >
            <DialogHeadlessui.Panel className={`
                inline-block max-h-screen w-screen md:w-3/5 lg:w-3/5 xl:w-1/2 p-8
                transform bg-white shadow-xl rounded-2xl flex flex-col`}>
              <DialogHeadlessui.Description
                as='div'
                className={classNames({ 'bg-edit': dialogType === DialogType.FORM, 'bg-white': dialogType === DialogType.DETAILS }, 'overflow-auto py-4 px-8 flex flex-col gap-3')}
              >
                ...
                {closeButton && (
                  <div className='flex justify-end px-8 py-4 mt-8 text-xl' >
                    <button
                      ref={initialFocusRef}
                      type='button'
                      onClick={onClose}
