...
import {
  $createLineBreakNode, $createParagraphNode, $createTextNode, $getRoot, $getSelection, $isParagraphNode
} from 'lexical'
import { $isListItemNode, $isListNode } from '@lexical/list'
import { $isQuoteNode } from '@lexical/rich-text'
import { $findMatchingParent } from '@lexical/utils'
import { IS_APPLE_WEBKIT, IS_IOS, IS_SAFARI } from '../../shared/environment'
import { isEmptyParagraph, PUNCTUATION_OR_SPACE, transformersByType } from './MarkdownUtils'
  ...
  return (markdownString, node) => {
    ...
    for (let i = 0; i < linesLength; i++) {
      ...
      if (imported) {
        ...
        // There could be other sub-markdown elements (both multiline and normal ones) matching within this matched
        // multiline element's children.

        // However, it would be the responsibility of the matched multiline transformer to decide how it wants to handle
        // them. We cannot handle those, as there is no way for us to know how to maintain the correct order of generated
        // lexical nodes for possible children.
...
/**
 *
 * @returns first element of the returned tuple is a boolean indicating if a multiline element was imported.
 * The second element is the index of the last line that was processed.
 */
  ...
  for (const transformer of multilineElementTransformers) {
    ...
    while (endLineIndex < linesLength) {
      const endMatch = regexpEndRegex
        ...
        : null
      if (!endMatch) {
        // Optional end, but didn't reach the end of the document yet => continue searching for potential closing match
        if (
          !isEndOptional ||
          (isEndOptional && endLineIndex < linesLength - 1)
        ) {
          endLineIndex++
      ...
      if (endMatch && startLineIndex === endLineIndex) {
        linesInBetween.push(
          lines[startLineIndex].slice(startMatch[0].length, -endMatch[0].length)
        )
      } else {
      ...
      if (
        ...
        // Return here. This $importMultiline function is run line by line and should only
        // process a single multiline element at a time.
        return [true, endLineIndex]
      }

      // The replace function returned false, despite finding the matching open and
      // close tags => this transformer does not want to handle it. Thus, we continue
      // letting the remaining transformers handle the passed lines of text from the beginning
      break
    }
  }
