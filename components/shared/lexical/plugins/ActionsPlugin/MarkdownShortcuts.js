...
import {
  $createRangeSelection, $getSelection, $isLineBreakNode, $isRangeSelection, $isRootOrShadowRoot, $isTextNode,
  $setSelection
} from 'lexical'
import invariant from 'shared/invariant'
import { $isCodeNode } from '@lexical/code'
import { TRANSFORMERS } from './LexicalMarkdown'
import { indexBy, PUNCTUATION_OR_SPACE, transformersByType } from './MarkdownUtils'
      ...
      for (const node of dependencies) {
        if (!editor.hasNode(node)) {
          invariant(
            false,
            `
              MarkdownShortcuts: missing dependency %s for transformer.
              Ensure node dependency is included in editor initial config.
            `,
            node.getType()
          )
        }
