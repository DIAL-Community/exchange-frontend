            ...
            return (
              <div key={`${country.name}-${aggregator.name}`} className='hover:bg-gray-100 hover:text-gray-900'>
                <div className='mx-4 py-2 border-b last:border-0'>
                  <a
                    href={`expand-${aggregator.slug}`} onClick={(e) => toggleExpand(e, aggregator.slug)}
                    className={`${active === aggregator.slug ? 'font-semibold' : ''} text-sm text-gray-700 cursor-pointer block`}
                  >
                    {aggregator.name}
                  </a>
                  {
                    active === aggregator.slug &&
                      <>
                        <a
                          href={aggregator.slug} onClick={(e) => openDetailPage(e, aggregator.slug)}
