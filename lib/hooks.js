...
export const useUser = () => {
  ...
  const isAdliAdminUser = data?.user?.isAdliAdminUser
  const loadingUserSession = status === 'loading'

  return { loadingUserSession, user, isAdminUser, isEditorUser, isAdliAdminUser }
}
...
export const useActiveTenant = () => {
  ...
  return { waitingActiveTenant, tenant, secured, hostname, editable, country }
}
