...
import { OPPORTUNITY_CATEGORY_INDICATORS_QUERY } from '../../shared/query/opportunity'
...
import { UPDATE_OPPORTUNITY_CATEGORY_INDICATORS } from '../../shared/mutation/opportunity'
      ...
      <AccordionItemHeading className='bg-dial-spearmint text-dial-stratos hover:bg-dial-mint'>
        <AccordionItemButton>
          ...
          <div className='h5 ml-auto my-auto'>
            {`
              ${format('opportunity.maturity.categoryScore')}:
                ...
                <AccordionItemHeading className='bg-dial-spearmint text-dial-stratos hover:bg-dial-mint'>
                  <AccordionItemButton>
                    ...
                    <div className='h5 float-right inline'>
                      {`
                        ${format('opportunity.maturity.indicatorScore')}:
...
const OpportunityDetailMaturityScores = ({ slug, overallMaturityScore, maturityScoreDetails }) => {
  const { formatMessage } = useIntl()
  ...
  } = useQuery(OPPORTUNITY_CATEGORY_INDICATORS_QUERY, {
    variables: { slug },
    skip: !isAdminUser
  })

  const defaultCategoryIndicators = useMemo(() => {
    if (categoryIndicatorsData?.opportunity) {
      const assignedIndicators =
        categoryIndicatorsData.opportunity.opportunityIndicators?.map(
          ({
            indicatorValue,
            categoryIndicator: {
              slug,
      ...
      const notAssignedIndicators =
        categoryIndicatorsData.opportunity.notAssignedCategoryIndicators?.map(
          ({
            slug,
            name,
            indicatorType,
    ...
    return []
  }, [categoryIndicatorsData?.opportunity])
  ...
  const [updateOpportunityIndicators, { loading: isMutating, reset: resetMutation }] = useMutation(
    UPDATE_OPPORTUNITY_CATEGORY_INDICATORS,
    {
      onCompleted: (data) => {
        const { updateOpportunityIndicators: response } = data
        if (response?.opportunity && response?.errors?.length === 0) {
          refetchCategoryIndicators()
          setValidMaturityScores(sortMaturityScoreDetails(data.updateOpportunityIndicators.opportunity.maturityScoreDetails))
          setMaturityScore(data.updateOpportunityIndicators.opportunity.overallMaturityScore)
      ...
      {validMaturityScores?.length
        ? <div className='grid grid-cols-1 xl:grid-cols-3 gap-12'>
          <div className='text-dial-meadow'>
            <div className='flex flex-col gap-y-6'>
              <div className='text-lg text-center font-semibold'>
                {format('opportunity.maturity.overallScore')}
              </div>
          ...
          <div
            ...
            data-tooltip-content={format('opportunity.maturity.chartTooltip')}
          ...
          >
            <div className='flex flex-col w-full'>
              <div className='h4 inline mb-6'>{format('opportunity.maturity.detailLabel')}</div>
              <Accordion
        ...
        : <div className='text-sm pb-5 text-button-gray'>{format('opportunity.noMaturity')}</div>
      }
    </>
  )
...
export default OpportunityDetailMaturityScores
