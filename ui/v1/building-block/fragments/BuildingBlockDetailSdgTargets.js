...
import { UPDATE_BUILDING_BLOCK_SDG_TARGETS } from '../../shared/mutation/buildingBlock'
import { SDG_TARGET_SEARCH_QUERY } from '../../shared/query/sdgTarget'
import { useUser } from '../../../../lib/hooks'
import { ToastContext } from '../../../../lib/ToastContext'

const BuildingBlockDetailSdgTargets = ({ buildingBlock, canEdit, headerRef }) => {
  const { formatMessage } = useIntl()
  const format = useCallback((id, values) => formatMessage({ id }, values), [formatMessage])

  const client = useApolloClient()

  const [sdgTargets, setSdgTargets] = useState(buildingBlock.sdgTargets)
  const [isDirty, setIsDirty] = useState(false)

  const [updateBuildingBlockSdgTargets, { data, loading, reset }] = useMutation(UPDATE_BUILDING_BLOCK_SDG_TARGETS,{
    onError() {
      setIsDirty(false)
      setSdgTargets(buildingBlock?.sdgTargets)
      showToast(format('toast.sdgTargets.update.failure'), 'error', 'top-center')
      reset()
    },
    onCompleted: (data) => {
      const { updateBuildingBlockSdgTargets: response } = data
      if (response?.buildingBlock && response?.errors?.length === 0) {
        setIsDirty(false)
        setSdgTargets(response?.buildingBlock?.sdgTargets)
  ...
  const onCancel = () => {
    setSdgTargets(data?.updateBuildingBlockSdgTargets?.buildingBlock?.sdgTargets ?? buildingBlock.sdgTargets)
    setIsDirty(false)
  }
...
export default BuildingBlockDetailSdgTargets
