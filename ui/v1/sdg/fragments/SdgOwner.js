...
const SdgOwner = ({ sdg }) => {
  ...
  const { isSdgOwner } = useSdgOwnerUser(sdg, [], isAdminUser)
  ...
  useQuery(CANDIDATE_ROLE_QUERY, {
    ...
    onCompleted: (data) => {
      ...
      if (candidateRole?.rejected === null) {
        // Applying to be the owner of the sdg
        setOwnershipText('applied-to-own')
      }
    }
  })

  const [revealContact] = useLazyQuery(SDG_CONTACT_QUERY, {
    onCompleted: (data) => {
  ...
  const [applyAsOwner, { reset }] = useMutation(APPLY_AS_OWNER, {
    ...
    onCompleted: (data) => {
      ...
      if (!response?.candidateRole || response?.errors?.length > 0) {
        ...
        showToast(
          format('toast.applyAsOwner.submit.success', { entity: format('sdg.label') }),
          'success',
          'top-center'
        )
        setLoading(false)
  ...
  const onSubmit = () => {
    if (user) {
      ...
      applyAsOwner({
        variables: {
          entity: ObjectType.SDG,
          entityId: parseInt(sdg.id)
        },
        context: {
          headers: {
            'Accept-Language': locale,
  ...
  return (
    <div className='flex flex-col gap-y-4 py-3'>
      <div className='text-xs'>{format('sdg.owner')}</div>
      <div className='flex gap-3 text-xs text-dial-stratos'>
        <a
          href={`https://docs.dial.community/projects/sdg-registry/${locale}/latest/sdg_owner.html`}
          target='_blank'
          rel='noreferrer'
        >
          <div className='border-b border-dial-iris-blue'>{format('sdg.ownerLink')} ⧉</div>
        </a>
        {showApplyLink && (
          <>
      ...
      {user && sdg.haveOwner && (
        <div className='flex text-xs text-dial-plum font-semibold'>
          {contactState === CONTACT_STATES[0] &&
            ...
            >
              {format('sdg.owner.revealContact')}
            </button>
          }
