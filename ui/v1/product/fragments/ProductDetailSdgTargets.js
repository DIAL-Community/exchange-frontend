...
import { UPDATE_PRODUCT_SDG_TARGETS } from '../../shared/mutation/product'
import { SDG_TARGET_SEARCH_QUERY } from '../../shared/query/sdgTarget'
import { useUser } from '../../../../lib/hooks'
import { ToastContext } from '../../../../lib/ToastContext'

const ProductDetailSdgTargets = ({ product, canEdit, headerRef }) => {
  const { formatMessage } = useIntl()
  const format = useCallback((id, values) => formatMessage({ id }, values), [formatMessage])

  const client = useApolloClient()

  const [sdgTargets, setSdgTargets] = useState(product.sdgTargets)
  const [isDirty, setIsDirty] = useState(false)

  const [updateProductSdgTargets, { data, loading, reset }] = useMutation(UPDATE_PRODUCT_SDG_TARGETS,{
    onError() {
      setIsDirty(false)
      setSdgTargets(product?.sdgTargets)
      showToast(format('toast.sdgTargets.update.failure'), 'error', 'top-center')
      reset()
    },
    onCompleted: (data) => {
      const { updateProductSdgTargets: response } = data
      if (response?.product && response?.errors?.length === 0) {
        setIsDirty(false)
        setSdgTargets(response?.product?.sdgTargets)
  ...
  const onCancel = () => {
    setSdgTargets(data?.updateProductSdgTargets?.product?.sdgTargets ?? product.sdgTargets)
    setIsDirty(false)
  }
...
export default ProductDetailSdgTargets
