...
import { UPDATE_PROJECT_BUILDING_BLOCKS } from '../../shared/mutation/project'
...
const ProjectDetailBuildingBlocks = ({ project, canEdit, headerRef }) => {
  const { formatMessage } = useIntl()
  const format = useCallback((id, values) => formatMessage({ id }, values), [formatMessage])

  const client = useApolloClient()

  const [buildingBlocks, setBuildingBlocks] = useState(project.buildingBlocks)
  ...
  const [mappingStatus, setMappingStatus] = useState(
    mappingStatusOptions.find(({ value: mappingStatus }) =>
      mappingStatus === (project?.buildingBlocksMappingStatus)
    ) ?? mappingStatusOptions?.[0]
  )

  const [updateProjectBuildingBlocks, { loading, reset }] = useMutation(UPDATE_PROJECT_BUILDING_BLOCKS, {
    onError() {
      setIsDirty(false)
      setBuildingBlocks(project?.buildingBlocks)
      showToast(format('toast.buildingBlocks.update.failure'), 'error', 'top-center')
      reset()
    },
    onCompleted: (data) => {
      const { updateProjectBuildingBlocks: response } = data
      if (response?.project && response?.errors?.length === 0) {
        setIsDirty(false)
        setBuildingBlocks(response?.project?.buildingBlocks)
...
export default ProjectDetailBuildingBlocks
