...
import { PROJECT_CATEGORY_INDICATORS_QUERY } from '../../shared/query/project'
...
import { UPDATE_PROJECT_CATEGORY_INDICATORS } from '../../shared/mutation/project'
      ...
      <AccordionItemHeading className='bg-dial-spearmint text-dial-stratos hover:bg-dial-mint'>
        <AccordionItemButton>
          ...
          <div className='h5 ml-auto my-auto'>
            {`
              ${format('project.maturity.categoryScore')}:
                ...
                <AccordionItemHeading className='bg-dial-spearmint text-dial-stratos hover:bg-dial-mint'>
                  <AccordionItemButton>
                    ...
                    <div className='h5 float-right inline'>
                      {`
                        ${format('project.maturity.indicatorScore')}:
...
const ProjectDetailMaturityScores = ({ slug, overallMaturityScore, maturityScoreDetails }) => {
  const { formatMessage } = useIntl()
  ...
  } = useQuery(PROJECT_CATEGORY_INDICATORS_QUERY, {
    variables: { slug },
    skip: !isAdminUser
  })

  const defaultCategoryIndicators = useMemo(() => {
    if (categoryIndicatorsData?.project) {
      const assignedIndicators =
        categoryIndicatorsData.project.projectIndicators?.map(
          ({
            indicatorValue,
            categoryIndicator: {
              slug,
      ...
      const notAssignedIndicators =
        categoryIndicatorsData.project.notAssignedCategoryIndicators?.map(
          ({
            slug,
            name,
            indicatorType,
    ...
    return []
  }, [categoryIndicatorsData?.project])
  ...
  const [updateProjectIndicators, { loading: isMutating, reset: resetMutation }] = useMutation(
    UPDATE_PROJECT_CATEGORY_INDICATORS,
    {
      onCompleted: (data) => {
        const { updateProjectIndicators: response } = data
        if (response?.project && response?.errors?.length === 0) {
          refetchCategoryIndicators()
          setValidMaturityScores(sortMaturityScoreDetails(data.updateProjectIndicators.project.maturityScoreDetails))
          setMaturityScore(data.updateProjectIndicators.project.overallMaturityScore)
      ...
      {validMaturityScores?.length
        ? <div className='grid grid-cols-1 xl:grid-cols-3 gap-12'>
          <div className='text-dial-meadow'>
            <div className='flex flex-col gap-y-6'>
              <div className='text-lg text-center font-semibold'>
                {format('project.maturity.overallScore')}
              </div>
          ...
          <div
            ...
            data-tooltip-content={format('project.maturity.chartTooltip')}
          ...
          >
            <div className='flex flex-col w-full'>
              <div className='h4 inline mb-6'>{format('project.maturity.detailLabel')}</div>
              <Accordion
        ...
        : <div className='text-sm pb-5 text-button-gray'>{format('project.noMaturity')}</div>
      }
    </>
  )
...
export default ProjectDetailMaturityScores
